...
for frame_idx in range(seq.shape[-1]-1):
    ...
    if frame_idx in [1, 20, 40, 60, 80]:
        # cv2.imwrite("../output/q1.3-girl_{}".format(frame_idx), )
        if not os.path.exists("../output"):
            os.makedirs("../output")
        fig.savefig("../output/q1.3-girl-wcrt_{}.jpg".format(frame_idx), bbox_inches='tight')
    rect_data[frame_idx+1] = rect
np.savetxt("../output/girlseqrects-wcrt.npy", rect_data)
